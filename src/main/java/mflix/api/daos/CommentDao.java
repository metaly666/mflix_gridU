...
public class CommentDao extends AbstractMFlixDao {
    ...
    public Comment addComment(Comment comment) {
        ...
        try {
            ...
            System.out.println("Exception happened while adding new comment: " + e.getMessage());
        }
        return comment;
    ...
    public boolean updateComment(String commentId, String text, String email) {
        ...
        try {
            return commentCollection.updateOne(Filters.eq(new ObjectId(commentId)),
                    Updates.set("text", text)).getModifiedCount() > 0;
        } catch (Exception e) {
    ...
    public List<Critic> mostActiveCommenters() {
        ...
        Bson group = Aggregates.group("$email", Accumulators.sum("count", 1));
        commentCollection.aggregate(Arrays.asList(
                group, sort(Sorts.descending("count")),
                limit(20), out("top20")
        )).toCollection();
        for (Document doc : db.getCollection("top20").find()) {
            mostActive.add(new Critic(doc.get("_id").toString(),
                    Integer.parseInt(doc.get("count").toString())));
        }
        return mostActive;
    }
}
